...
```
Stage 1 — Path & Footstep Planner   (~2 Hz)
...
Stage 2 — Trajectory Optimiser      (~10 Hz)
  ...
  stage2_main.py        Stage 1 → Stage 2 entry point
```
...
```bash
uv run python main.py                   # demo world
uv run python main.py warehouse
uv run python main.py corridor
uv run python main.py assembly_line
```
...
All tunable constants sit at the top of `main.py` and `stage2_main.py`:
