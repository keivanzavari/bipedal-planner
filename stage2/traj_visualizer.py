...
def plot_trajectory_2d(
    ...
    s = 10
    ax.plot(traj.x[::s], traj.y[::s], "-",  color="#e67e22", linewidth=2,
            zorder=8, label="CoM trajectory")
    ax.plot(traj.zmp_x[::s], traj.zmp_y[::s], "--", color="#9b59b6", linewidth=1.5,
            zorder=8, label="ZMP trajectory")

    # Mark start and end
    ax.plot(traj.x[0],  traj.y[0],  "o", color="#e67e22", markersize=8,
            zorder=9, markeredgecolor="black")
    ax.plot(traj.x[-1], traj.y[-1], "s", color="#e67e22", markersize=8,
            zorder=9, markeredgecolor="black")
...
def plot_time_series(
    ...
    def shade_phases(ax):
        ...
        for k, kind in enumerate(schedule.kind):
            ...
            if is_double != in_double:
                if in_double:
                    ax.axvspan(t_start, t[k], alpha=0.12, color="#3498db",
                               label="Double support" if t_start == t[0] else "")
                in_double = is_double
                t_start   = t[k]
        if in_double:
    ...
    axes[0].set_title("Stage 2 — CoM & ZMP time series  "
                       "(shaded = double support)")
